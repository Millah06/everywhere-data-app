...
const purchaseSecureElectric = async (req: any, res: any) => {
  try {
    const {
      ...
      plan,
      serviceID,
      meterNumber, meterType, quantity
    } = req.body;

    if (!phoneNumber || !amount || !network  || !clientRequestId || !meterNumber || !plan || !serviceID || !meterType) {
    ...
    await admin.firestore().runTransaction(async (t) => {
      ...
      const bonusPercent = bonusDoc.data()?.cable;
    ...
    await admin.firestore().runTransaction(async (t) => {
      ...
      const calculation = calculateTransaction({
        ...
        useReward,
        isRecharge,
        bonusPercent: bonusDoc.data()?.electric,
      });
