 ...
 * Authenticated. Verifies the OTP sent for password reset — does NOT change password yet.
 * Body: { otp }
 */
export const verifyPasswordOtp = async (req: any, res: any) => {
  try {
    ...
    const user = await prisma.user.findUnique({
      ...
      select: { id: true, passwordResetOtpHash: true, passwordResetOtpExpiry: true },
    });
